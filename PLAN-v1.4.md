...
>
> ⚠️ **后续修订见 `PLAN-v1.5.md`**:v1.5 对 v1.4 交付的内容做了事实核验,发现并修复了
> 40 条与官方原文不符的数据(其中 W-C 引入的 HGX B300 有 2 条高危文案问题)与 4 个界面缺陷
> ——**其中一个缺陷正是由本批次追加第五个系统触发的**:汇报页 §04 按 `systems` 声明顺序
> 机械串联,HGX B300 被追加到 `systems[4]` 后就被排成了「NVL576 → LPX → HGX B300」的
> 最新一代。本文件的门禁数字(616 单测 / E2E 43+43)是 v1.4 收工时的真实记录,不作追改;
> v1.5 终态为 **667 单测 / E2E 45 passed + 45 skipped**。
